...
describe('SearchScreen', () => {
  ...
  it('multi-token query uses name search only', async () => {
    ...
    expect((Cocktails as any).__mock.searchByName).toHaveBeenCalledWith(
      'gin fizz',
    );
  ...
  it('single non-generic word intersects ingredient ∩ name, with backfill if intersection small', async () => {
    ...
    expect((Cocktails as any).__mock.filterByIngredient).toHaveBeenCalledWith(
      'collins',
    );
  ...
  it('clicking Search trims input and triggers search', async () => {
    ...
    const btn = screen
      .getAllByRole('button')
      .find((b) => within(b).queryByText('Search'));
  ...
  it('caps results to ≤ 60 after ranking even if API returns many', async () => {
    ...
    const rendered = screen
      ...
      .filter((t: any) => /^Gin Forward|^Something/.test(t.props?.children));
