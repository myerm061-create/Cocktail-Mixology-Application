...
export function ingredientImageUrl(name: string, size: IngredientImageSize = "Small"): string {
  const safe = encodeURIComponent(name.trim());
  if (size === "Large") return `https://www.thecocktaildb.com/images/ingredients/${safe}.png`;
